...
object GenerateCityFile extends IOApp {
  ...
  def readCountries[F[_]: Sync](path: Path): F[Map[String, String]] = {

    def toEntry(line: String) = {
      val pair = line.split(',')
      if (pair.length != 2) None else (pair(0) -> pair(1)).some
    }

    def readLine(reader: BufferedReader, accum: Map[String, String]): F[Map[String, String]] =
      for {
        line <- Sync[F].delay(reader.readLine())
        result <- if (line != null) readLine(reader, accum ++ toEntry(line))
  ...
  def inputReader[F[_]: Sync](f: Path): Resource[F, BufferedReader] =
    Resource.make {
  ...
  def writer[F[_]: Sync](f: Path): Resource[F, BufferedWriter] =
    Resource.make {
  ...
  def generateCityFile[F[_]: Sync](inputPath: Path, outPath: Path, countries: Map[String, String]): F[Int] = {
    ...
    } yield (inStream, outStream)).use {
      case (in, out) => processLines(in, out, 0)
    }
  }
